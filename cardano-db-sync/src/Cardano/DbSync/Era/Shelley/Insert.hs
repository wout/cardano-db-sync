...
--
-- We can't have TxOut here, because other TxIn in the same block (different Tx)
-- may reference them. So they can't be missing from the DB. We could include
-- them.
...
_insertTxIn
    ...
    -> (Generic.TxIn, DB.TxId, DbLovelace)
    -> ExceptT SyncNodeError (ReaderT SqlBackend m) ()
_insertTxIn _tracer txInId redeemers (Generic.TxIn _txHash index txInRedeemerIndex, txOutId, _lovelace) = do
...
_insertMaTxOut
    ...
    -> ExceptT SyncNodeError (ReaderT SqlBackend m) ()
_insertMaTxOut _tracer txOutId maMap =
